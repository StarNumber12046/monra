...
export const create = (x: string) => {
    ...
    if (!x[1]) {
       ...
       if (x[2] == "-ts" || x[2] == "--typescript") {
           rl.question('\x1b[2m[1/2]\x1b[0m \x1b[1mPackage Name:\x1b[0m (@'+monra?.name+'/'+x[1]+') ', (name: any) => {
            if (!name) name = "@"+monra?.name+"/"+x[1];
            rl.question("\x1b[2m[2/2]\x1b[0m \x1b[1mVersion:\x1b[0m (1.0.0) ", (v: any) => {
                if (!v) v = "1.0.0";
                ...
                fs.writeFileSync("monra.json", JSON.stringify(monra))
                ...
                console.log("\x1b[32m[monra]\x1b[0m Create package!");
                rl.close()
            }) 
          });
       } else {
        rl.question('\x1b[2m[1/2]\x1b[0m \x1b[1mPackage Name:\x1b[0m (root) ', (name: any) => {
            if (!name) name = "@"+monra?.name+"/"+x[1];
            rl.question("\x1b[2m[2/2]\x1b[0m \x1b[1mVersion:\x1b[0m (1.0.0) ", (v: any) => {
                if (!v) v = "1.0.0";
                fs.mkdirSync(monra?.directory+"/"+x[1]);
