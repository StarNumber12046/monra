...
export const run = (x: string) => {
  ...
  if (x[1] == "--global" || x[1] == "-g") {
    ...
    const yesorno = question(
      "\x1b[2m[monra]\x1b[0m \x1b[1mAre you sure you want to run the command\x1b[0m (y/n) "
    );
    ...
    if (yesorno == "y" || yesorno == "yes" || yesorno == "") {
      ...
      files.map((file) => {
        console.log(
          "\x1b[33m[monra]\x1b[0m Running command in \x1b[1m" +
            monra?.directory +
            "/" +
            file +
            "\x1b[0m, pls wait..."
        ...
        const res = execSync(
          "cd " + monra?.directory + "/" + file + " && " + command
        );
    ...
    if (yesorno == "y" || yesorno == "yes" || yesorno == "") {
      const packageint = JSON.parse(
        fs.readFileSync(monra?.directory + "/" + x[1] + "/package.json", "utf8")
      );
      console.log(
        "\x1b[33m[monra]\x1b[0m Running command in \x1b[1m" +
          monra?.directory +
      ...
      const res = execSync(
        "cd " + monra?.directory + "/" + x[1] + " && " + command
      );
      console.log(res);
      console.log("\x1b[32m[monra]\x1b[0m Done!");
    }
